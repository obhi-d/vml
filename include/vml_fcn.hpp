...
inline float arc_tan(float value) { return std::atan(value); }
inline float arc_tan2(float y, float x) { return std::atan2(y, x); }
inline float arc_sin(float value) { return std::asin(value); }
inline float arc_cos(float value) { return std::acos(value); }
inline float abs(float value) { return std::fabs(value); }
inline float sqrt(float val) { return std::sqrt(val); }
inline float recip_sqrt(float val) { return 1.f / std::sqrt(val); }
...
inline bool is_pow2(std::uint32_t val) { return (val & (val - 1)) == 0; }
//-------------------------------------------------------
// inlined functions
...
inline std::uint16_t float_to_half(float f) { return float_to_half_i(*(std::uint32_t*)&f); }
...
inline float to_radians(float value) { return k_degrees_to_radian_factor * (value); }

inline float to_degrees(float value) { return (value) * 57.295779513f; }
