...
inline quad::type quad::isinfv(quad::pref v)
{
  ...
  type vtemp = _mm_and_ps(v, vml_cast_i_to_v(_mm_set_epi32(0x7fffffff, 0x7fffffff, 0x7fffffff, 0x7fffffff)));
  // Compare to infinity
  vtemp = _mm_cmpeq_ps(vtemp, vml_cast_i_to_v(_mm_set_epi32(0x7F800000, 0x7F800000, 0x7F800000, 0x7F800000)));
...
inline quad::type quad::abs(quad::pref q)
{
  ...
  return _mm_and_ps(q, vml_cast_i_to_v(_mm_set_epi32(0x7fffffff, 0x7fffffff, 0x7fffffff, 0x7fffffff)));
#else
  return quad::set(vml::abs(q[0]), vml::abs(q[1]), vml::abs(q[2]), vml::abs(q[3]));
#endif
}
...
#endif
#else
  return quad::set(vml::recip_sqrt(qpf[0]), vml::recip_sqrt(qpf[1]), vml::recip_sqrt(qpf[2]), vml::recip_sqrt(qpf[3]));
#endif
}
...
{
  return _mm_movemask_epi8(vml_cast_v_to_i(_mm_cmpneq_ps(a, b))) == 0;
}
...
inline bool operator<(__m128 const& a, __m128 const& b) noexcept
{
  return _mm_movemask_epi8(vml_cast_v_to_i(_mm_cmpnlt_ps(a, b))) != 0;
}

inline bool operator>(__m128 const& a, __m128 const& b) noexcept
{
  return _mm_movemask_epi8(vml_cast_v_to_i(_mm_cmpngt_ps(a, b))) != 0;
}

inline bool operator<=(__m128 const& a, __m128 const& b) noexcept
{
  return _mm_movemask_epi8(vml_cast_v_to_i(_mm_cmpngt_ps(b, a))) == 0;
}

inline bool operator>=(__m128 const& a, __m128 const& b) noexcept
{
  return _mm_movemask_epi8(vml_cast_v_to_i(_mm_cmpnlt_ps(b, a))) == 0;
}
