...
TEST_CASE("Validate mat4::mul", "[mat4::mul]") {
	...
	CHECK(vml::vec4::equals(vml::mat4::mul(vml::mat4::row(m1, 0), m2),
	                        vml::mat4::row(m1m2, 0)));
}
...
TEST_CASE("Validate mat4::transform_assume_ortho",
	...
	CHECK(vml::vec3a::equals(vml::vec3a::from_vec4(vml::mat4::transform_assume_ortho(
	                             m, vml::vec3a::set(3.000f, 10.000f, 12.000f))),
...
TEST_CASE("Validate mat4::transform_and_project",
	...
	CHECK(vml::vec3a::equals(
	    vml::mat4::transform_and_project(m,
	                                     vml::vec3a::set(3.000, 10.000, 12.000)),
...
TEST_CASE("Validate mat4::transform_aabb", "[mat4::transform_aabb]") {
	...
	vml::mat4_t combined =
	    vml::mat4::mul(scale, vml::mat4::mul(rotate, translate));
	vml::aabb_t expected =
	    vml::aabb::set(vml::vec3a::set(10, 0, 0), vml::vec3a::set(4, 8, 4));
...
TEST_CASE("Validate mat4::from_perspective_projection",
	...
	vml::mat4_t proj = vml::mat4::from_perspective_projection(vml::k_pi_by_2,
	                                                          1.2f, 1.0f, 100.0f);
	float y_scale =
	    1.0f / std::tan(static_cast<float>(3.14159265358979323846 / 4.0));
...
TEST_CASE("Validate mat4::inverse", "[mat4::inverse]") {
	...
	vml::mat4_t inv = {
	    -0.232841581f, 0.0817205757f, 0.255250692f,   -0.0759970918f,
	...
	vml::mat4_t o = {
	    0.0f,   0.80f, 0.60f,  0.0f, -0.80f, -0.36f, 0.48f, 0.0f,
	    -0.60f, 0.48f, -0.64f, 0.0f, 12.0f,  20.0f,  3.0f,  1.0f,
	};

	vml::mat4_t oi = {0,   -0.8f, -0.6f,  0, 0.8f,   -0.36, 0.48f,  0,
	                  0.6f, 0.48f, -0.64f, 0, -17.8f, 15.3600016f, -0.48f, 1};
